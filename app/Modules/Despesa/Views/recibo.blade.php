...
<head>
    <meta charset="UTF-8">
    <title>Comprovante Pagamento #{{ $despesa->id }}</title>
    <style>
...
<div class="recibo-title">COMPROVANTE DE PAGAMENTO Nº {{ str_pad($despesa->id, 6, '0', STR_PAD_LEFT) }}</div>

<div class="section-title">Dados da despesa</div>
<table class="dados">
    <tr>
        <td class="label">Descrição:</td><td>{{ $despesa->nome }}</td>
        <td class="label">Documento:</td><td>{{ $despesa->documento ?? '—' }}</td>
    </tr>
    <tr>
        <td class="label">Categoria:</td><td>{{ $despesa->categoria->descricao ?? '—' }}</td>
        <td class="label">Status:</td><td class="{{ $status === 'paga' ? 'pago' : 'pendente' }}">{{ ucfirst($status) }}</td>
    </tr>
    <tr>
...
<table class="dados">
    <tr>
        <td class="label">Nome:</td><td colspan="3">{{ $despesa->fornecedor_nome ?? '—' }}</td>
    </tr>
</table>
...
@if($despesa->observacoes)
    ...
    <div style="font-size: 10pt;">{{ $despesa->observacoes }}</div>
@endif
